...
function real_login_db($db_conn, $uid, $passwd) {
    ...
    $query = "SELECT login_id FROM db_authentication WHERE login_name = " .
    real_mysql_specialchars($uid, false) . " AND password='" . md5($passwd) . "'";
    $results = real_execute_query($query, $db_conn);
    if (mysql_num_rows($results) > 0) {
        //real_update_login_success ( $db_conn, $uid, 1 );
        return true;
    }else {
        //real_update_login_success ( $db_conn, $uid, 0 );
        real_set_error($_err_login);
        real_redirect_onerror($_err_page, "", $db_conn);
    }
}
/* function real_update_login_success($db_conn, $uid, $success) {

  $query = "INSERT INTO gen_login_info (login_id, last_datetime, success) values(" . real_mysql_specialchars ( $uid, false ) . ", NOW(), $success)
  ON DUPLICATE KEY UPDATE ";
  $result = real_execute_query ( $query, $db_conn );

  } */
function real_get_login_id_by_login_name_db($db_conn, $login_name) {
    $login_name_esc = real_mysql_specialchars($login_name, false);
    $query = "SELECT login_id FROM db_authentication WHERE login_name = $login_name_esc";
    $results = real_execute_query($query, $db_conn);
    if (mysql_num_rows($results) > 0) {
...
function real_get_ppl_by_login_id_db($db_conn, $login_id) {
    ...
    $login_id_esc = real_mysql_specialchars($login_id, false);
    $query = "SELECT $_db_ppl_output FROM db_user_info WHERE login_id= $login_id_esc ";
    return real_get_first_row($db_conn, $query);
}
function real_get_ppla_by_login_id_db($db_conn, $login_id) {
    global $_db_ppla_output;
    $login_id_esc = real_mysql_specialchars($login_id, false);
    $query = "SELECT $_db_ppla_output FROM db_user_info WHERE login_id= $login_id_esc ";
    return real_get_first_row($db_conn, $query);
}
function real_get_pplb_by_login_id_db($db_conn, $login_id) {
    global $_db_pplb_output;
    $login_id_esc = real_mysql_specialchars($login_id, false);
    $query = "SELECT $_db_pplb_output FROM db_user_info WHERE login_id= $login_id_esc ";
    return real_get_first_row($db_conn, $query);
}
...
function real_get_ppl_by_name_db($db_conn, $last_name, $first_name, $page_no) {
    global $_db_ppl_output;
    $last_name_esc = mysql_real_escape_string($last_name);
    $first_name_esc = mysql_real_escape_string($first_name);
    $clause = " FROM db_user_info WHERE l_name LIKE '$last_name_esc%' AND f_name LIKE '$first_name_esc%' ORDER BY l_name, f_name ";
...
function real_get_actual_page_no($page_no, $count_entries) {
    ...
    if ($count_entries <= (($page_no - 1) * $_browse_search_rows_per_page)) $page_no = $max_page;
    $retval [0] = $page_no;
    $retval [1] = $max_page;
    return $retval;
}
