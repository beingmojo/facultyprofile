...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	{
		$query = 	"DELETE FROM gen_association USING gen_association, fac_technology
						WHERE gen_association.pid = fac_technology.tech_pid 
						AND gen_association.assoc_pid = fac_technology.pid 
						AND fac_technology.add_req = 0 
						AND fac_technology.pid = " . real_mysql_specialchars( $pid, true );
	...
	{
		$query = "UPDATE fac_technology SET del_req = 1 " .
					" WHERE add_req = 0 AND pid= " . real_mysql_specialchars( $pid, true );
		real_execute_query( $query, $db_conn );		
	}
