...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	$name = substr( real_unescape( $_POST["fac_equipment_name"] ), 0, 255 );
	$cost = substr( real_unescape( $_POST["fac_equipment_cost"] ), 0, 32 );
	$value = real_unescape( $_POST["fac_equipment_value"] );
	$url_name = substr( real_unescape( $_POST["fac_equipment_url_name"] ), 0, 255 );
	$url = substr( real_unescape( $_POST["fac_equipment_url"] ), 0, 255 );
	$url = real_validate_url($url);
	$ipFilter = new InputFilter(array( "script" ),"",1,1,1);
	$description = $ipFilter->process( $_POST["fac_equipment_description"] );
	$status = ($_POST["fac_equipment_status"]=="on")?"1":"0";
	$remove_image = ($_POST["fac_equipment_remove_image"]=="on")?"1":"0";
	...
	if( $name != "" )
	{
		$query = "UPDATE fac_equipment SET " .
