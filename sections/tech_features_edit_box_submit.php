...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	$abstract = $ipFilter->process( $_POST["tech_features_abstract"] );
	$testquery="SELECT * FROM tech_features WHERE pid=".real_mysql_specialchars( $pid, true);
	$testresult=real_execute_query($testquery, $db_conn);
	$isthere=mysql_num_rows($testresult);
	if($isthere==1)
		$query = "UPDATE tech_features SET description=" . real_mysql_specialchars( $abstract, false ) .
				 " WHERE pid=". real_mysql_specialchars( $pid, true );
	else
		$query = "INSERT INTO tech_features(description, pid) VALUES ( ".
