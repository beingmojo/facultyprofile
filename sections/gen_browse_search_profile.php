 ...
 */
$dept_esc = real_mysql_specialchars(stripslashes($_GET['deptsearch']), true);
...
if( $_GET["type"] == 'pplc' )
{
    ...
    $fromclause = " FROM ppl_general_info T1, gen_profile_info T2 ";
	$whereclause = " WHERE T2.status = 0 AND T1.pid = T2.pid AND T2.type_id=1 ";
         
        if(strlen($fname_esc)!=0)
        {
		$whereclause = $whereclause." AND T1.f_name LIKE '".$fname_esc."%'";
        }
        if(strlen($lname_esc)!=0)
        {
		$whereclause = $whereclause." AND T1.l_name LIKE '".$lname_esc."%'";
        }
        if (isset($dept_esc) && $dept_esc != 0){
		//$fromclause = $fromclause . ", gen_hierarchy_types T3 ";
		$whereclause = $whereclause . "  AND (T1.pri_hid = " . $dept_esc." OR T1.hid LIKE '%".$dept_esc."%')";
	}

		//if (isset($dept_esc))
    //    $clause = $clause." AND T3.hid=" . $dept_esc ;
	$clause = $fromclause.$whereclause . " ORDER BY T1.l_name, T1.f_name, T1.m_name";
}
...
{
    $fld_query = "SELECT CONCAT( T1.title, IF(T1.title<>'', ' ', '') , T1.l_name, IF(T1.f_name<>'', ' ', ''), T1.f_name, IF(T1.m_name<>'', ' ', '') , T1.m_name ) as name, T1.pri_designation as designation, T1.pid, T1.login_id, T1.image_id, T1.title, T1.l_name, T1.f_name, T1.m_name, T1.phone_no_1, T1.phone_no_2, T1.email_id, T1.status_phone_no_1, T1.status_phone_no_2,T1.keywords, T1.status_email_id, T1.mailbox, T1.office_location, T1.room_no, T1.status_mail_address, T1.pri_hid ";
 $clause = "FROM ppl_general_info T1, gen_profile_info T2, gen_hierarchy_types T3 WHERE T2.status = 0 AND T1.pid = T2.pid AND T2.type_id=1 AND T3.hid=" . $dept_esc ;
...
if( $_GET["type"] == 'tech' )
{
    ...
    $clause = " FROM tech_gen_info T1, tech_abstract T2, gen_profile_info T3 WHERE T1.pid = T2.pid AND T1.pid = T3.pid AND T3.status =1 AND T3.type_id=3 ";
    if(strlen($name_esc)!=0){
        $clause = $clause." AND T1.name LIKE '%".$name_esc."%'";
    }
    $clause = $clause." ORDER BY T1.name";
...
error_log( "Running query:".$fld_query . $clause . $limit."\n\n", 0, "/tmp/php-errors.log");
...
$counter = 0;
$type  = $_GET["type"];
if(  $type=='ppl' || $type=='ppla' || $type=='pplb' || $type=='pplc' || $type == 'ppld' )
{
   print "<$type maxpage='" . $max_page . "' pageno='". $pageno . "'>";
    while($rows=mysql_fetch_array($fld_results))
    {
        ...
        if( $type == 'ppla' || $type == 'pplb' || $type == 'pplc' || $type == 'ppld' )
        {
        ...
        if( $type == 'pplb' || $type == 'pplc' || $type == 'ppld' )
        {
