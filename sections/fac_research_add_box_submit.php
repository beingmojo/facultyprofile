...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	$itr = 0;
	$name = substr( real_unescape( $_POST["fac_research_name"] ), 0, 255 );
	$url_name = substr( real_unescape( $_POST["fac_research_url_name"] ), 0, 255 );
	$url = substr( real_unescape( $_POST["fac_research_url"] ), 0, 255 );
	$url = real_validate_url($url);
	$ipFilter = new InputFilter(array( "script" ),"",1,1,1);
	$description = $ipFilter->process( $_POST["fac_research_description"] );
	$status = ($_POST["fac_research_status"]=="on")?"1":"0";
	$image_id = real_insert_image( $db_conn, $pid, $section_id, "imagefile", "../images" );

	if( $name != "" )
	{ 
		$itr ++;
		$query = "INSERT INTO fac_research ( pid, resch_id, name, url_name, url, description, status, image_id ) VALUES( " .
