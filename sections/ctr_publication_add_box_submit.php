...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	for( $i = 1; $i <= 3; $i++ )
	{
		$year = substr( real_unescape( $_POST["ctr_publication_year".$i] ), 0, 16 );
		$ranking = real_unescape( $_POST["ctr_publication_ranking".$i] );
		$ipFilter = new InputFilter($_filter_tags,"",1,1,1);
		$name = $ipFilter->process( $_POST["ctr_publication_name".$i] );
		$url = substr( real_unescape( $_POST["ctr_publication_url".$i] ), 0, 255 );
		$url = real_validate_url($url);
		$category = substr( real_unescape( $_POST["ctr_publication_category".$i] ), 0, 32 );
		if( $category == "" )
			$group_by = substr( real_unescape( $_POST["ctr_publication_group_by".$i] ), 0, 32 );
		else
			$group_by = $category;
		...
		{ 
			$itr ++;
			$query = "INSERT INTO ctr_publication ( pid, pub_id, year, ranking, name, url, group_by, status ) VALUES( " .
					$pid .
					", " . ($max_pub_id + $itr) . 
