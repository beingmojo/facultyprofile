/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/prototype.js */
...
/*--------------------------------------------------------------------------*/
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/scriptaculous/effects.js */
...
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/scriptaculous/sound.js */
...
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/gato-lib.js */
...
function record_video_analytics (videourl) {
	if (destroyerGlobalPageTracker) { try { destroyerGlobalPageTracker._trackEvent('Videos', videourl, document.title+' <'+window.location+'>'); } catch (e) {} }
	if (destroyerSitePageTracker) { try { destroyerSitePageTracker._trackEvent('Videos', videourl, document.title+' <'+window.location+'>'); } catch (e) {} }
}
...
function finish_video_load(lnk, options, captUrl) {
	...
	if (captUrl) {
		...
		options.clip.onStart = function () {
			...
			if (readCookie('txst_caption_persist') > 0) toggle_captions(lnk);
...
function video_load(lnk, opts) {
	...
	lnk = $(lnk);
	if (!opts) opts = {};
	if (flashembed && flashembed.isSupported([9, 115]) && !lnk.flowLoaded) {
		...
		if (opts.captUrl) {
			finish_video_load(lnk, options, opts.captUrl);
		} else {
			var capt = lnk.next('a.videoframe-caption-link');
			if (capt && capt.readAttribute('href')) {
				finish_video_load(lnk, options, capt.readAttribute('href'));
			} else {
				var tryurl = lnk.href.replace(/\.\w*$/,'.srt');
				var abortTimer;
				new Ajax.Request(process_caption_url(tryurl), {
					method: 'get',
					onCreate: function (t) {
						abortTimer = setTimeout(function () {
							t.transport.abort();
							finish_video_load(lnk, options, '');
						}, 600);
					},
					onSuccess: function (t) {
						clearTimeout(abortTimer);
						if (t.responseText.length > 5) {
							finish_video_load(lnk, options, tryurl);
						} else {
							finish_video_load(lnk, options, '');
						}
					}
...
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/txstate/js/khan-outgoingclicks.js */
	...
	$$('a').each( function(item) {
		item.observe('click', function() {
			...
			if ( typeof( destroyerSitePageTracker ) != 'undefined') {
				destroyerSitePageTracker._trackEvent('Links', thisPageTitle + " <" + thisPageAddress + ">", linkName + " <" + linkAddress + ">" );
			}
			if ( typeof( destroyerGlobalPageTracker ) != 'undefined') {
				destroyerGlobalPageTracker._trackEvent('Links', thisPageTitle + " <" + thisPageAddress + ">", linkName + " <" + linkAddress + ">" );
			}
		});
	});
});
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/flowplayer/flowplayer-3.1.4.min.js */
...
/* IMPORTED FROM FILE: /opt/magnolia/webapps/magnoliaAssets/common/js/ie6-deprecation.js */
...
var domain = "." + hostElements[ hostElements.length - 2 ] + "." + hostElements[ hostElements.length - 1 ];
if ( isIE6 ) {
	var DEPRECATION_COOKIE_KEY = "bigOlStinkyStringThatNobodyWouldUseEverForIE6DeprecationWarnings";
	if ( readCookie ( DEPRECATION_COOKIE_KEY ) == null ) {
		alert("Texas State University's websites use a number of advanced features which aren't well-supported by Internet Explorer 6. For the best experience, please update your browser to a current version.");
		createCookie( DEPRECATION_COOKIE_KEY, "upgrade", domain, 7 );
	}
}
