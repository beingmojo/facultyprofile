...
{
	//alert("inside find");
	var uri = "../browse_search_course.php?fname=" + firstname + "&lname=" + lastname + "&pageno=" + pageno;
...
function processXML(obj, tbl)
{
	...
	for (var i = 0; i < root.childNodes.length; i++) 
	{
		...
		searchResultsTable += "<td>" + fpResultsSearch[i][0] + "</td><td>" + fpResultsSearch[i][1] + "</td><td>" + fpResultsSearch[i][8] + 
			"</td><td>" + fpResultsSearch[i][9] + "</td><td style=\"display:none\">" + fpResultsSearch[i][3] + "</td>";
...
function Selected(tbl, i)
{
	if (tbl == "copiResults")
	{
		...
		document.getElementById("ciRank").value = fpResultsSearch[i][1];
...
function mtdc(year)
{
	...
	var stdRate = 0.48;
	var fieldNames = new Array('bu_ss','bu_tuition','bu_equip', 'bu_ps', 'bu_pTrav');
	var field;
	var reqMTDC;
			...
			for(var k=1; k<=5; k++)
			{
				...
				idcTotal += parseFloat(document.getElementById('bu_idc'+k).value);
				tcTotal += parseFloat(document.getElementById('bu_tc'+k).value);
			}
			reqMTDC = mtdcTotal*stdRate;
			reqMTDC = Math.round(reqMTDC*100)/100;	
			idcTotal=Math.round(idcTotal*100)/100;
			document.getElementById('bu_reqMTDC').value = reqMTDC.toString();
			...
			document.getElementById('bu_tcTotal').value = tcTotal.toString();
			document.getElementById('bu_lIDC').value = (reqMTDC-idcTotal).toString();
}
...
function AddRow(tblName)
{
	...
	if (tblName == "CoPis")
	{
		...
		var id = newRow.id;
		newCell.innerHTML = "<span onclick='RemoveRow(\"" + tblName + "\",\"" + id + "\")'><img src=\"images/deleterow.gif\" alt=\"Remove\"></span>" +
...
function HandlePrintPreview(data)
{
	...
	customcategories = "";
	copissig = "";
	copiSigValue = '<tr><td colspan="5"><br><br><br></td></tr><tr><td width="30%"><table style="border-top: 1px solid #000000;" width="100%"><tr><td align="left" width="80%">Co-Principal Investigator</td><td width="20%" align="right">Date</td></tr></table></td><td width="5%"></td><td width="30%"><table style="border-top: 1px solid #000000;" width="100%"><tr><td align="left" width="70%">Chairperson</td><td width="30%" align="right">Date</td></tr></table></td><td width="5%"></td><td width="30%"><table style="border-top: 1px solid #000000;" width="100%"><tr><td align="left" width="70%">Dean/Director</td><td width="30%" align="right">Date</td></tr></table></td></tr>';
	copiColNum = 1;
	extiColNum = 1;
...
function Save(sstatus)
{
	if (sstatus == 2)
	{
		if (!(checkSection6() && checkSection10()))
		{
			submitted=false;
			return;
		}
