...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	{
		$tech_pid = $_POST[ $boxname . "_" . $itr . "_2" ]; 
		if( real_check_user_groupid( $db_conn, "admin" ) || real_check_user_groupid( $db_conn, "tech_admin" ) )
		{
			$query = "INSERT IGNORE INTO fac_technology ( pid, tech_pid, add_req ) VALUES( " .
					real_mysql_specialchars($pid, true) .
					", " . real_mysql_specialchars( $tech_pid, true ) .
					",0  ) ON DUPLICATE KEY UPDATE add_req = 0";
	
			real_execute_query( $query, $db_conn );
			
			$query = "INSERT IGNORE INTO gen_association (pid, assoc_pid) VALUES ( " . real_mysql_specialchars( $tech_pid, true ) . ", " . real_mysql_specialchars( $pid, true ) . ") ";
			real_execute_query( $query, $db_conn );
		}
		else
		{
			$query = "INSERT IGNORE INTO fac_technology ( pid, tech_pid ) VALUES( " .
					real_mysql_specialchars($pid, true) .
					", " . real_mysql_specialchars( $tech_pid, true ) .
					"  )";
