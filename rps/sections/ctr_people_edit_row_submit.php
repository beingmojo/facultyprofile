...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	$title = substr( real_unescape( $_POST["ctr_people_title"] ), 0, 8 );
	$f_name = substr( real_unescape( $_POST["ctr_people_f_name"] ), 0, 255 );
	$m_name = substr( real_unescape( $_POST["ctr_people_m_name"] ), 0, 255 );
	$l_name = substr( real_unescape( $_POST["ctr_people_l_name"] ), 0, 255 );
	$rank = substr( real_unescape( $_POST["ctr_people_rank"] ), 0, 255 );
	$login_id = substr( real_unescape( $_POST["ctr_people_login_id"] ), 0, 255 );
	
	if( $f_name != ""  &&  $l_name != "" )
	{
		$query = "UPDATE ctr_people SET " .
