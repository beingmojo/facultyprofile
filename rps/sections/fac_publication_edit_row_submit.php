...
if( real_can_user_edit( $db_conn, $pid ) == true )
{
	...
	$year = substr( real_unescape( $_POST["fac_publication_year"] ), 0, 16 );
	$ranking = real_unescape( $_POST["fac_publication_ranking"] );
	$ipFilter = new InputFilter($_filter_tags,"",1,1,1);
	$name = $ipFilter->process( $_POST["fac_publication_name"] );
	$url = substr( real_unescape( $_POST["fac_publication_url"] ), 0, 255 );
	$url = real_validate_url($url);
	$category = substr( real_unescape( $_POST["fac_publication_category"] ), 0, 32 );
	if( $category == "" )
		$group_by = substr( real_unescape( $_POST["fac_publication_group_by"] ), 0, 32 );
	else
		$group_by = $category;
	...
	{ 
		$itr ++;
		$query = "UPDATE fac_publication SET " .
