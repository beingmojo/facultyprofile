...
{	
	...
	{		
		...
		$link = "http://www.uta.edu/ra/real/editprofile.php?pid=$pid&onlyview=1";
		...
		//Absolute URL required
			$p= "http://www.uta.edu/ra/real/images/48/". $pid . "_0_" . $image["$pid"] .".jpg";
			$document = $document .  $p;			
		}
		$document = $document . '</imageurl>';
		...
		if (($type["$pid"] == 2)||($type["$pid"] == 4)||($type["$pid"] == 5)||($type["$pid"] == 6))
		{
			...
			if ($owner_cols = mysql_fetch_array ($owner_searchresults))
			{
				...
				$proflink = "http://www.uta.edu/ra/real/editprofile.php?pid=".$owner_cols["pid"]."&onlyview=1";
