...
if($course_acronym||$course_number||$course_semester||$course_year||$course_title||$course_description||$prof_fname||$prof_lname) {
	if ($course_acronym)
		$searchSQL = $searchSQL . "t2.acronym LIKE '$course_acronym%' AND ";
	if ($course_number)
		$searchSQL = $searchSQL . "t1.course_number LIKE '$course_number%' AND ";
	if ($course_semester)
		$searchSQL = $searchSQL . "t1.semester LIKE '$course_semester%' AND ";
	if ($course_year)
		$searchSQL = $searchSQL . "t1.year LIKE '$course_year%' AND ";
	if ($course_title)
		$searchSQL = $searchSQL . "(t1.course_title LIKE '$course_title%' OR t1.course_title LIKE '$course_titleUpper%' OR t1.course_title LIKE '$course_titleFUpper%') AND ";
	if ($course_description)
		$searchSQL = $searchSQL . "MATCH (t1.description) AGAINST ('\"$course_description\"' IN BOOLEAN MODE) AND ";
	if ($prof_fname)
		$searchSQL = $searchSQL . "t3.f_name LIKE '$prof_fname%' AND ";
	if ($prof_lname)
		$searchSQL = $searchSQL . "t3.l_name LIKE '$prof_lname%' AND ";
...
function printPageLinks($totalRows,$rowsPerPage,$currPage) {
	if($totalRows%$rowsPerPage == 0)
		...
		$pages = $totalRows/$rowsPerPage + 1;
	for($i = 1; $i <= $pages; $i++)
		if($i == $currPage)
			echo "<span style=\"font-size:14px;font-weight:bold;color:#000000;\">".$i."</span> ";
		else
			printf("<a class=\"font_topic\" href=\"javascript:document.getElementById('page').value='%d';ajaxCourseBrowse();\">%d</a> ",
					$i,$i);
}
...
while($searchRow = mysql_fetch_array($searchQuery)) {
	...
	if($searchRow['course_url'])
		$courseLink = sprintf("<a href=\"%s\" target=\"_blank\">Course Homepage</a>%s",$searchRow['course_url'],$searchRow['file_name']?" | ":"");
	else
		$courseLink = "";
	if($searchRow['file_name'])
		$syllabusLink = sprintf("<a href=\"%s%s\" target=\"_blank\">Course Syllabus</a><br />[Uploaded:%s | File size:%.2fkb]",
						$searchRow['upload_location'],$searchRow['file_name'],$searchRow['upload_time'],$searchRow['file_size']*.001);
	else
		$syllabusLink = "";
	if (file_exists($_SERVER['DOCUMENT_ROOT'].$_home."/images/48/".$searchRow['pid']."_0_".$searchRow['image_id'].".jpg"))
		$imageSrc = "images/48/".$searchRow['pid']."_0_".$searchRow['image_id'].".jpg";
	else
		$imageSrc = "images/icons/silhouette_small.jpg";
...
if ($rowCounter === 0)
	echo "<tr><td style=\"font-size:12px;\">No Matches Found.</td></tr>";
echo "</tbody>";
echo "</table>";
?>