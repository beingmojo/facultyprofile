...
elseif($_GET['search']) {
    ...
    while($section = mysql_fetch_array($sectionsQuery))
        $section_filter[] = htmlspecialchars($section['table_name']);
}
...
if ($_POST['search_str_less'])
    $search_str = $search_str.eregi_replace('([^- ][a-zA-Z]+)([ ]|$)',' -\\1\\2',$_POST['search_str_less'])." ";
        ...
        if($tables[$i] != "equipment") { //Exception for equipment search
            ...
            if ($field_filter[$tables[$i]]) {
                foreach($field_filter[$tables[$i]] as $field)
                    $fulltextFields = $fulltextFields.$field.",";
            ...
            if($defaultFields === true)
                $searchSQL = "SELECT pid, MATCH($fulltextFields) AGAINST ('$search_str') AS score, ";
            else
                $searchSQL = "SELECT pid, SUM(MATCH($fulltextFields) AGAINST ('$search_str' IN BOOLEAN MODE)) AS score, ";
...
for($i = 0; $i < count($searchQueries); $i++) {
    if (mysql_num_rows($searchQueries[$i]['query']) > 0) {
        while($pidResult = mysql_fetch_row($searchQueries[$i]['query'])) {
                ...
                for ($wordCounter = 0; $wordCounter < count($searchWordsSplit); $wordCounter++) {
                    ...
                    if ($regMatch != false && $pidMatchedString[$pid] == false) {
                        ...
                        if ($position < 40)
                            ...
                            $startPos = $position - 40;
                        if (strlen($matchedString) - $startPos < 200)
                            $maxLength = strlen($matchedString) - $startPos;
                        else
                            $maxLength = 200;
            ...
            if(!$pidSectionNames[$pid])
                $pidSectionNames[$pid] = array();
            if(!in_array($sectionResult['name'],$pidSectionNames[$pid]))
                $pidSectionNames[$pid][] = $sectionResult['name'];
            //Set section ID for output
            if(!$pidSectionIds[$pid])
                $pidSectionIds[$pid] = array();
            if(!in_array($sectionResult['section_id'],$pidSectionIds[$pid]))
                $pidSectionIds[$pid][] = $sectionResult['section_id'];
        }
    }
}
...
if(count($pids) != 0) {
    arsort($pidScores);
    $sortedPids = array();
    foreach($pidScores as $index => $pidScore)
        $sortedPids[] = $pids[$index];
    $pids = $sortedPids;
}
?>
...
<?php  
        ...
        ?>
        <title>Research Profile System - (RPS Advanced Search) - Texas State University - San Marcos</title>
        <!-- InstanceEndEditable -->
        ...
        <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF" class="form_elements_row_action" id="basic_layout">
            ...
            <tr>
                <td width='30%' align="left" class="table_background_other"><!-- InstanceBeginEditable name="pagename" -->
                    <div align="left" class="page_heading">rSp Advanced Search: <?php echo count($pids)." matches found."; ?></div>
...
//Print search results
                    ...
                    foreach($pids as $pid) {
                        ...
                        if($profileInfoResult['image_id'] != 0 && file_exists($_SERVER['DOCUMENT_ROOT']."/ra/real/".$imgSrc))
                            printf("<img src=\"%s\" alt=\"%s\"/>",$imgSrc,htmlspecialchars($profileInfoResult['name'], ENT_QUOTES));
                        else
                            echo "&nbsp;";
                        echo "</td>";
                        ...
                        if ($imgsrc != "")
                            printf("<img src=\"%s\" alt=\"%s\" />",$imgsrc,$alt);
                        echo "</td>";
                        //Print institution icon cell
                        echo "<td valign=\"middle\" align=\"right\"  width=\"5%\">";
                        if ($profileInfoResult['owner_login_id'] == "other1" || $profileInfoResult['owner_login_id'] == "other2")
                            echo "<img src=\"images/unt_logo.gif\" alt=\"UNT\" />";
                        else
                            echo "<img src=\"images/uta_logo_black.gif\" alt=\"UTA\" />";
                        echo "</td>";
                        echo "</tr>";
                    }
                    if (count($pids) == 0) {
                        ...
                        for($i=0;$i<count($searchWordsSplit);$i++) {
                            $subWords = "";
                            for($ii=0;$ii<$totalWords;$ii++)
                                $subWords = $subWords . "+".substr($searchWordsSplit[$ii],0,4)."* ";
                        ...
                        if(strlen($suggest) > 0)
                            printf("<p style=\"font-size:16px;margin-bottom:5px;\">Did you mean <a href=\"advsearchresults.php?search=%s\">%s</a>?</p>",
                        ...
                        if(!in_array($user_ip,$noInsertIp) && !$_GET['search'])
                            $insertRecentSearch = real_execute_query("INSERT INTO gen_recent_search VALUES (now(),'$search_str_esc','$user_ip_esc')",$db_conn);
