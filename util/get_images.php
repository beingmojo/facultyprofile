...
function returnimages($dirname="../images/128") {
    $db_conn = mysql_connect("localhost", "ys11", "profilesystempass");
    mysql_select_db("profilesystem", $db_conn);
            ...
            if (eregi($pattern, $file)) { //if this file is a valid image
                ...
                if (mysql_num_rows($result) == 0) {
                    ...
                    if (list($r_int) = mysql_fetch_array($result)) {
                        $r_int = str_replace(",", "", $r_int);
                        $r_int = str_replace("\n", " ", $r_int);
                        $r_int = str_replace("\r", " ", $r_int);
                        if (strlen($r_int) > 120) {
                            $r_int = substr($r_int, 0, 120);
                            $r_int .= "...";
                        }
                        $echoString .= $r_int;
                    }else {
