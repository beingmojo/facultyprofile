...
if ( isset($_GET['route']) )
{
	...
	"\n ------------------------------------------------------------------------------- \n" .
	"Technical Questions/Difficulties call 512-245-2102 or email sv1117@txstate.edu . \n" .
	...
	"Other Blue-sheet related questions email sv1117@txstate.edu or contact your specialist (http://www.txstate.edu/research/osp). \n\n" .
	...
	if($row = mysql_fetch_array($result_i))
	{
		...
		mail( $copi_email, $email_subject, $message, "From: OSP<sv1117@txstate.edu>" );
	}
	...
	while($row = mysql_fetch_array($result_i))
	{
		...
		if ($row1 = mysql_fetch_array($result_i1))
		{
			...
			if ($row2 = mysql_fetch_array($result_i2))
			{
				...
				mail( $dean_email, $email_subject, $message, "From: OSP<sv1117@txstate.edu>" );
			}
			// get the chair
			...
			if ($row2 = mysql_fetch_array($result_i2))
			{
				...
				mail( $chairperson_email, $email_subject, $message, "From: OSP<sv1117@txstate.edu>" );
			}
		}
	}
	...
	while($row123 = mysql_fetch_array($result_i123))
	{
		...
		else
		{
			...
			mail( "EHandS@institution.edu", $email_subject, $message, "From: OSP<sv1117@txstate.edu>" );
		}
	}
...
if ($handle_copi_routing)
{
	...
	$email_text .= 	"Please login at http://{$_SERVER['HTTP_HOST']}{$_home}/loginscreen.php?view=2 to review this bluesheet. \n " . 
	"\n ------------------------------------------------------------------------------- \n" .
	"Technical Questions/Difficulties call 512-245-2334 or email sailaja@txstate.edu . \n" .
	"Looking for help? Check out blue sheet quick start guide: \n" . 
	"Word Document: http://{$_SERVER['HTTP_HOST']}{$_home}/help/bluesheet_quickstart_guide.doc \n" .
	"PDF: http://{$_SERVER['HTTP_HOST']}{$_home}/help/bluesheet_quickstart_guide.pdf \n" .
	"Other Blue-sheet related questions email sv1117@txstate.edu or contact your specialist (http://www.txstate.edu/research/osp.html). \n\n" .
	"If you are receiving multiple emails for the same bluesheet, please submit a preferred email address to sv1117@txstate.edu & we will add it for future routing.";
	$email_subject = "New bluesheet submitted";
...
if ($sstatus != "")
{
  ...
  if ($sstatus == "Submitted")
  {
	  $message = "Dear Specialists: \n\n" . 
		...
		mail( "sv1117@txstate.edu", "Pending Bluesheet [$pi_dept]", $message, "From: OSP<sv1117@txstate.edu>" );
  }
}
else
{
...
$fieldnames = array('Salary', 'Fringe Benefits', 'M&O - Materials', 'Consulting Services (non-Texas State)',
...
if (isset($route))
{
	...
	while($row123 = mysql_fetch_array($result_i123))
	{
		...
		else
		{
			...
			$result_i2 = real_execute_bs_query($query, $connect) or die("05 - " . mysql_error());
			$message = "Dear Sir/Madam: \n\n" . $email_text;
			mail( "EHandS@institution.edu", $email_subject, $message, "From: OSP<sv1117@txstate.edu@institution.edu>" );
		}
	}
