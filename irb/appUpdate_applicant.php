...
///////////////////////////////////////////////////////////////////////
 mysql_select_db($database_con3, $con3);
 $query = sprintf("SELECT User_Type FROM user WHERE username = '%s'", $rs_app['username']);
$app1 = mysql_query($query, $con3) or die(mysql_error());
$row_app1 = mysql_fetch_assoc($app1);
$totalRows1 = mysql_num_rows($app1);
$applicantType = $row_app1['User_Type'];
mysql_free_result($app1);
?>
...
if (i==0) {
	...
	//	}
	altert("You have not submitted any documentation. You must at least have a synopsis. You cannot submit application now.");
	return false;
	}
if (i>0) {
...
<table width="800" border="0" align="center" bgcolor="#ffffff">
  <tr>
    <td bgcolor="#000000"><div align="center"><a class="irb" href="<?php echo $_SESSION['myhome'];?>">My IRB Home</a> <span class="style42">|</span> <a class="irb" href="
<?php 
...
?>
?appNum=<?php echo $rs_app['App_Number'];?>">Application Status</a> <span class="style42">|</span> <a class="irb" href="<?php if(($_SESSION['User_Type'] == 'IRB Chair') || ($_SESSION['User_Type'] == 'IRB Staff')) echo "appSummary_irb.php"; 
else echo "appSummary_applicant.php";?>
?appNum=<?php echo $rs_app['App_Number'];?>">Review Application Data</a><?php 
...
?><p>If you believe that you have completed the  application form correctly and you are finished uploading all supporting  documents, click Submit Application button below. Once you click the Submit Application button, you will not be able to make changes, and your application will be forwarded for review.<p>
	 ...
	 ?>
	  <input name="save" type="button" value="Save Application and Return Home" onClick="returnHome()"></form>
     </label>
...
<form name="IRBForm" ENCTYPE="multipart/form-data" method="POST" action = "<?php echo $editFormAction; ?>" >
  ...
  <blockquote> 
    ...
    <input type="radio" name="four_Project_Type" value="Institutional/Admin" <?php if($rs_app['four_Project_Type']=="Institutional/Admin") echo "checked";?>>
 ...
 <?php if (strtoupper($applicantType)=="STUDENT")
 {
 ?>
<p>
  If you are a student, please provide the following informaiton about the faculty member that you work with on this project: </p>
  <table width="690" border="0" align="center" bgcolor="#ccddcc">
    <tr>
      <td>Faculty First Name:        </td>
      <td><input name="FacultyFirstName" type="text" id="FacultyFirstName" value="<?php echo $rs_app['FacultyFirstName']; ?>"></td>
      <td>Faculty Last Name:        </td>
      <td><input name="FacultyLastName" type="text" id="FacultyLastName" value="<?php echo $rs_app['FacultyLastName']; ?>"></td>
    </tr>
    <tr>
      <td>Faculty Email Address:        </td>
      <td><input name="FacultyEmail" type="text" id="FacultyEmail" value="<?php echo $rs_app['FacultyEmail']; ?>"></td>
      <td>Faculty Phone Number:        </td>
      <td><input name="FacultyPhone" type="text" id="FacultyPhone" value="<?php echo $rs_app['FacultyPhone']; ?>"></td>
    </tr>
    <tr>
      <td colspan="4">Department/Office: 
        <input name="FacultyDepartment" type="text" id="FacultyDepartment" size="50" value="<?php echo $rs_app['FacultyDepartment']; ?>"></td>
      </tr>
    <tr>
      <td colspan="4">Has your faculty representative/chair reviewed and approved your research and all related documents? (e.g., consent form, synopsis, survey, etc.) 
        <input name="FacultyApproval" type="radio" value="Yes" <?php if($rs_app['FacultyApproval']=="Yes") echo "checked";?>>
        Yes 
         <input name="FacultyApproval" type="radio" value="No" <?php if($rs_app['FacultyApproval']=="No") echo "checked";?>>
        No</td>
      </tr>
  </table>
  <?php
...
die("<tr><td><font color='red'>You cannot update the application at this moment. The application is locked. Please contact the IRB Administrator or Chair to unlock this application. <a href='javascript:history.back()'>Back</a></font></td></tr>");
?>
  <?php
