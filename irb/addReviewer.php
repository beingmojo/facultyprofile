...
$query_Recordset3 = "SELECT `user`.username,`user`.FirstName, `user`.LastName,`user`.Department, `user`.User_Type FROM `user` WHERE (`user`.User_Type = 'reviewer' && (username != '".$theValue1."' && username != '".$theValue2."')) order by LastName" ;
}
  if ($noRev==1){
  $theValue = (!get_magic_quotes_gpc()) ? addslashes($row_Recordset1['rev1ID']) : $row_Recordset1['rev1ID'];
$query_Recordset3 = "SELECT `user`.username,`user`.FirstName, `user`.LastName,`user`.Department, `user`.User_Type FROM `user` WHERE (`user`.User_Type = 'reviewer' && (username != '".$theValue."')) order by LastName" ;

}
 if ($noRev==0)
$query_Recordset3 = "SELECT `user`.username,`user`.FirstName, `user`.LastName,`user`.Department, `user`.User_Type FROM `user` WHERE `user`.User_Type = 'reviewer' order by LastName" ;
...
if(isset($_POST['insert']) && $_POST['insert'] == "formInsert") 
 {
  ...
  $newaction=$_POST['irbActionLog']."<br>".$today."  ".$_SESSION['username'].":<br>Add a Reviewer to the application. ";
  //The user ID of this reviewer is <em>". $_POST['userID']."</em>";
 
 $email1="";
 $email2="";
 $email3="";
if(strtoupper(trim($_POST['trainingFinished'])) != "YES")
...
//if (strpos($row_Recordset2['Email'], $substring) == false) {
  ...
  // echo("<p><font color='red'>The reviewer does not have a Texas State University email address. Click Notice Reviewer to inform him/her that an IRB application has been assigned to him/her. <a href='mailto:".$row_Recordset2['Email']."?subject=Review IRB application: ".$appNum."'>Notice Reviewer</a></font>");
mysql_free_result($Recordset2);
//}
}
?>
...
$query_Recordset4 = "SELECT application.App_Number, application.ProjectTitle, rev1Comments, rev2Comments,rev3Comments FROM application WHERE ((application.rev1ID = '".$theValue."' || application.rev2ID = '".$theValue."' || application.rev3ID = '".$theValue."') && (Status != 'Approved' && Status != 'Applicant Requested Continuation' && Status != 'Application Approved - Exempt'))";
$Recordset4 = mysql_query($query_Recordset4, $con) or die(mysql_error());
$row_Recordset4 = mysql_fetch_assoc($Recordset4);
$totalRows_Recordset4 = mysql_num_rows($Recordset4);
echo " (total assigned: ".$totalRows_Recordset4. ", ";

// Revison status
$query_Recordset5 = "SELECT application.Status FROM application WHERE ((application.rev1ID = '".$theValue."' || application.rev2ID = '".$theValue."' || application.rev3ID = '".$theValue."') && (application.Status = 'IRB Chair Requests Revision'))";
$Recordset5 = mysql_query($query_Recordset5, $con) or die(mysql_error());
...
}while($row_Record4=mysql_fetch_assoc($Recordset4));
$countCommented = $totalRows_Recordset4 - $countNoComment;
