...
$newaction=$_POST['irbActionLog']."<br><br>".$today."  faculty member ".$_SESSION['name'].":<br>";
 $newaction = $newaction."reviewed the application.";
 if (strtoupper($_POST['facultyCertified']) == "YES"){
//$sql = sprintf("update application set facultyCertified = %s, Status=%s,  irbActionLog=%s WHERE App_Number = %s", $_POST['facultyCertified'], "Application Certified by Faculty", $newaction, $_POST['appNum']);

  $sql = sprintf("UPDATE application SET facultyCertified = %s, facultyReviewDate=%s,irbActionLog=%s, Status='%s' WHERE App_Number=%s", GetSQLValueString($_POST['facultyCertified'], "text"),GetSQLValueString($today, "text"), GetSQLValueString($newaction,"text"), "Application Certified by Faculty", GetSQLValueString($_POST['appNum'], "text"));
//echo $sql;
$result = mysql_query($sql, $con) or die(mysql_error());
}
 if (strtoupper($_POST['facultyCertified']) == "NO"){
 $sql = sprintf("UPDATE application SET facultyCertified = %s, facultyReviewDate=%s, irbActionLog=%s, Status='%s' WHERE App_Number=%s", GetSQLValueString($_POST['facultyCertified'], "text"), GetSQLValueString($today, "text"), GetSQLValueString($newaction,"text"),"Application in Process", GetSQLValueString($_POST['appNum'], "text"));
$result = mysql_query($sql, $con) or die(mysql_error());
}

if (strtoupper($_POST['facultyCertified']) == "YES")
{
 $subject = "IRB Application ". $_POST['appNum'].": Application Approved by faculty";
  
  $body = "DO NOT REPLY TO THIS MESSAGE. This email message is generated by the IRB online application program.\r\r\nThe faculty has reviewed the student's application  and has approved the submission. The applicantion is now ready for review. The application number is: ". trim($_POST['appNum']).".\r\rYou can click the following link to log into IRB Online Application System:\r\n".$irbadd."?appNum=".$_POST['appNum']."\r\n";
...
if (strtoupper($_POST['facultyCertified']) == "NO"){
$subject = "IRB Application ". $_POST['appNum'].": Application rejected by faculty";
  
  $body = "DO NOT REPLY TO THIS MESSAGE. This email message is generated by the IRB online application program. Your supervising faculty member has reviewed your IRB application and has not approved the submission. Please contact your faculty member regarding revisions, then log back into this application and make the necessary changes. If you need to upload revised documents, rename them to avoid confusion with the original documents. Please revise this application, do not begin a new one. 
\r\n The application number is: ". trim($_POST['appNum']).".\r\rThe status of your application has been reset to Application in Process to allow access. You can click the following link to log into IRB Online Application System: \r\n".$irbadd."?appNum=".$_POST['appNum']."\r\nIf you have questions or need assistance, please contact me. ";
...
$username=$rs_app['username'];
//echo "<br>".$rs_app['FacultyEmail']."   ".$_SESSION['Email']."<br>";
if (strtoupper($rs_app['FacultyEmail']) != strtoupper($_SESSION['Email'])) {
//die("You are not authorized to review this application.");
 //header("Location: ". $restrictGoTo); 
 // exit;
}
mysql_select_db('Connections/dbc.php');
$query_Recordset2 = "SELECT Email FROM user where username='".$username."'";
...
<table width="800" border="0" align="center" bgcolor="#ffffff">
  <tr>
    <td bgcolor="#000000"><div align="center"><a class="irb" href="<?php echo $_SESSION['myhome'];?>">My IRB Home</a> <span class="style42">|</span> <a class="irb" href="list_student_app.php">Application Submitted by Students</a> |</span> <a href="LogOut.php" class="irb">Log Out</a>
...
?>
	...
	if ($i==0)
	echo "<p><font color='red'>The applicant has not submitted any documentation. </font> At a minimum, supporting documentation must include a Synopsis and if applicable, a Consent Form. In addition, other types of supporting documents include surveys or questionnaires, subject recruitment materials, approvals from other IRBs or from external sites, etc. 
<br>";
	}?>
	<?php if ($rs_app['Status']=="Application Submitted"){ ?>
    <form name="form1" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
     <input name="irbActionLog" type="hidden" value="<?php echo $rs_app['irbActionLog']; ?> " />
      <p>
        <label>
          <input type="radio" name="facultyCertified" value="Yes" <?php if ($rs_app['facultyCertified'] == 'Yes') echo "checked";?>>
          Yes. I certify that I have reviewed my student's application to the IRB and I approve the submission. My student and I have both completed CITI training. I understand that the application will not be reviewed by the IRB until I complete this step and that I will be copied on all correspondence  between the student and the IRB.</label><br>
        <label>
          <input type="radio" name="facultyCertified" value="No" <?php if ($rs_app['facultyCertified'] == 'No') echo "checked";?>>
          No. I do not approve the submission. The status for the application will be changed to "Application in Process". My student will revise the application and resubmit it (not start a new application).</label>
        <br></p>
      <label>      </label>
      <p align="center">
        <label>
		<input type="hidden" name="emailAdd" value="<?php echo $emailAdd; ?>">
		 ...
		 <input type="hidden" name="certify" value="certify">
    ...
    <P>
    <hr>
  <span class="style41">  <b>Application status: </b><?php 
  ...
  echo $rs_app['Status']; 
  if ($rs_app['Status'] != "Application in Process"){
  ?>
  
  </font><p><b>Submission approved by faculty member?</b> <?php echo $rs_app['facultyCertified']; 
  }
  ?>
  </p>
  </span>
