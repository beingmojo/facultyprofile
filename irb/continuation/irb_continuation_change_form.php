...
$query_Recordset1 = "SELECT `user`.Email FROM `user` where (User_Type = 'IRB Staff' || User_Type='IRB Chair')";
...
$errmsg = "";
       ...
       if(trim($_POST['conOrChange']) =="" )
   {
      $errmsg = $errmsg.'You must check if your application is a continuation of or a change to the original IRB application\r';
   }
...
//Good for this year
$query_Recordset2 = "SELECT App_Number, numContinued, expired, lastApprovalDate FROM application where App_Number = '".$_POST['App_Number']."'";
$Recordset2 = mysql_query($query_Recordset2, $con3) or die(mysql_error());
$row_Recordset2 = mysql_fetch_assoc($Recordset2);
$totalRows_Recordset2 = mysql_num_rows($Recordset2);
...
mysql_select_db($database_con3, $con3);
$query_Recordset3 = "SELECT * FROM continuation where (App_Number = '".$_POST['App_Number']."' and conOrChange <> 'Change')";
$Recordset3 = mysql_query($query_Recordset2, $con3) or die(mysql_error());
$row_Recordset3 = mysql_fetch_assoc($Recordset2);
$totalRows_Recordset3 = mysql_num_rows($Recordset2);
if ($totalRows_Recordset3 >=2){
echo "You have applied for continuation for this project two times. You can not make this request anymore. <a href='javascript:history.back()'>Back</a>";
mysql_free_result($Recordset3);
exit;
}
	$exprationDate=date("m/d/y", strtotime($actionRequestDate) + 365*60*60*24);
	$timeNow=date("m/d/y");
	if ($timeNow > $exprationDate +3){
	echo "Your application is expired. You cannot request for continuation anymore.";
	mysql_free_result($Recordset3);
	exit;
}
if ($_POST['conOrChange'] =="Continuation"){
$numContinued = $row_Recordset2['numContinued']+1;
}
$appFinished= "No";
$approval="No";
  $insertSQL = sprintf("INSERT INTO continuation (appFinished, ID, App_Number, GrantIDNumber, LengthOfProject, IRBDecisionHardCopy, StudyStatus, StudyStatusExplanation, NumberOfParticipantsApproved, ParticipantsEnrolledSinceLastReview, ParticipantsEnrolledToDate, DifferentEnrollmentExplanation, RelationshipChange, PIChange, PIChangeExplanation, ResultsSummary, UnanticpatedProblems, RiskBenefitChange, RiskBenefitChangedExplanation, ChangesInStudySinceApproval, ReceiveDate,Approval, username, conOrChange) VALUES (%s, %s,%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s,%s, %s)",
  					GetSQLValueString($appFinished, "text"),
					   ...
					   GetSQLValueString($_POST['conOrChange'], "text")
					   );
 ...
 if ($_POST['conOrChange'] == "Change"){
 $status = "Applicant Requested Change";
 }
 else {
  $status = "Applicant Requested Continuation";
  }

  $insertSQL = sprintf("UPDATE application SET Status=%s, continuationID=%s, numContinued=%s WHERE App_Number=%s", GetSQLValueString($status, "text"), GetSQLValueString($_POST['ID'], "text"), GetSQLValueString($numContinued, "text"), GetSQLValueString($_POST['App_Number'], "text"));
   mysql_select_db($database_con3, $con3);
  $Result1 = mysql_query($insertSQL, $con3) or die(mysql_error());
  }
  /////////////////////////////////////////////////////////////////////
...
$subject = "IRB Application Continuation/Change ".$_POST['ID']." -  submitted for review";
  $body = "DO NOT REPLY TO THIS MESSAGE. This email message is generated by the IRB online application program. \r\rAn IRB Continuation/Change request has been submitted for review. Continuation request number: ".$_POST['ID']. ". Original IRB Application Number: <a href='".$irbadd."?appNum=".$_POST['App_Number']."'>".$_POST['App_Number']."</a>";
  $body = $body.$emailSig;
  do{
...
<input type="hidden" name="appNum" value="<?php echo $_POST['App_Number']; ?>" id="appNum">
...
$directory = "appdoc/".$appNum."/continuation";
  ...
  <form action="<?php echo $editFormAction; ?>" method="POST" name="form1" class="style46">
    ...
    <p align="center" class="style47"><strong>APPLICATION FOR IRB CONTINUATION/CHANGE </strong></p>
	 <p>This is your IRB Continuation/Change Application Number: <font color="#CC0033"><?php echo $ID;?></font></p><hr />
         <p ><font color="red">Is this application a change to or a continuation of the original IRB application?</font><br>
      
        <input type="radio" name="conOrChange" value="Continuation">
        Continuation
      
        <input type="radio" name="conOrChange" value="Change">
        Change
      <br></p>
    <hr>
...
&nbsp;&nbsp;&nbsp; </p>
  ...
  <p align="center">
    <input name="Submit" type="submit" value="Submit and Upload Document" />
    </form>
    </td>
