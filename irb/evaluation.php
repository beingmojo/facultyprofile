...
if(trim($_POST['rev1ID']) == $_SESSION['username']){
	...
	if (strtoupper($_POST['finishReview']) == "YES"){
		...
		$updateSQL = "update reviewlog set rev1Finished='Yes', rev1Approved ='".$_POST['approve']."' where (reviewNum='". $_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
		...
		$updateSQL = "update reviewlog set rev1Finished='No', rev1Approved ='".$_POST['approve']."' where (reviewNum='". $_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
                    mysql_select_db($database_con3, $con3);
			$result = mysql_query($updateSQL, $con3);
...
$updateSQL = sprintf("UPDATE application SET rev1Comments=%s, rev1Approved=%s, rev1ApprovedDate=%s, irbActionLog=%s WHERE App_Number=%s",		
   ...
   ////////////////////////////////////////////////////////////
	...
	if (strtoupper($_POST['finishReview']) == "YES"){
		...
		$updateSQL = "update reviewlog set rev2Finished='Yes', rev2Approved ='".$_POST['approve']."' where (reviewNum='". $_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
		...
		//$updateSQL = "update reviewlog set rev2Finished='".$_POST['finishReview']."', rev2Approved ='".$_POST['approve']."' where (reviewNum='".$_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
		$updateSQL = "update reviewlog set rev2Finished='No', rev2Approved ='".$_POST['approve']."' where (reviewNum='". $_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
...
$updateSQL = sprintf("UPDATE application SET rev2Comments=%s, rev2Approved=%s, irbActionLog=%s WHERE App_Number=%s",
   ...
   //////////////////////////////////////////
	 ...
	 if (strtoupper($_POST['finishReview']) == "YES"){
		...
		$updateSQL = "update reviewlog set rev3Finished='Yes', rev3Approved ='".$_POST['approve']."' where (reviewNum='".$_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
	      mysql_select_db($database_con3, $con3);
			$result = mysql_query($updateSQL, $con3);
	  	}
               else{

		$updateSQL = "update reviewlog set rev3Finished='No', rev3Approved ='".$_POST['approve']."' where (reviewNum='".$_POST['reviewnum']."' && appNum='".$_POST['App_Number']."')";
   mysql_select_db($database_con3, $con3);
			$result = mysql_query($updateSQL, $con3);
	  	}
	   if((strtoupper(trim($_POST['approve'])) =="YES")||(trim($_POST['approve']) =="Approved pending changes")){
...
if($numReviewers == 2){
if((strtoupper($row_result['rev1Finished'])=="YES") && (strtoupper($row_result['rev2Finished'])=="YES")){
...
//if((strtoupper($row_result['rev1Finished'])=="NO") || (strtoupper($row_result['rev2Finished'])=="NO")){
//echo "chair need to be noticed";

//testing code
//$subject = "IRB Application ".$_POST['App_Number'].": Reviewers have evaluated the application. One or more of the reviewers has not finished the review.";
//$body = "IRB Application ".$_POST['App_Number'].": Reviewers evaluated the application";
...
<br>
        ...
        <p><span class="body"><span class="style44">*</span></span>Have you finished reviewing the application? The IRB Chair will not see your comments if you do not indicate that you have finished the review of the application. 
          Yes<input name="finishReview" type="radio" id="finishReview" value="Yes"
                                  <?php if (($row_Recordset1['rev1ID'] == $_SESSION['username']) && strtoupper($row2['rev1Finished']) =="YES") echo "checked";
