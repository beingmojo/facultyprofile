...
$today=date("m/d/y H:i:s");
  $newaction=$_POST['irbActionLog']."<br><br>".$today."  ".$_SESSION['username'].":<br>Assigned reviewer(s). Reviewer userID: ";
...
if (strpos($to, $substring) !== false) {
   ...
   echo("The reviewer does not have a Texas State University email address. Click here to inform him/her that an IRB application has been assigned to him/her. <a href='mailto:".$to."'>Notice Reviewer</a>");
}
...
$str="Application Submitted to Reviewers for Review";
  ...
  $insertSQL = sprintf("UPDATE application SET irbActionLog=%s, Status = %s, ReviewDate = %s, rev1ID=%s, rev2ID=%s, rev3ID=%s WHERE App_Number=%s", GetSQLValueString($newaction, "text"), GetSQLValueString($str, "text"), GetSQLValueString($today, "text"),GetSQLValueString($_POST['userID'][0], "text"), GetSQLValueString($_POST['userID'][1], "text"), GetSQLValueString($_POST['userID'][2], "text"), GetSQLValueString(trim($_POST['appNum']), "text"));
}
...
$str="Application Submitted to Reviewers for Review";
  ...
  $insertSQL = sprintf("UPDATE application SET irbActionLog=%s, Status = %s, ReviewDate = %s, rev1ID=%s, rev2ID=%s WHERE App_Number=%s", GetSQLValueString($newaction, "text"),GetSQLValueString($str, "text"), GetSQLValueString($today, "text"), GetSQLValueString($_POST['userID'][0], "text"), GetSQLValueString($_POST['userID'][1], "text"),  GetSQLValueString(trim($_POST['appNum']), "text"));
}
 
 if ($c==1){
$str="Application Submitted to Reviewers for Review";

  $insertSQL = sprintf("UPDATE application SET irbActionLog=%s, Status = %s, ReviewDate = %s, rev1ID=%s WHERE App_Number=%s", GetSQLValueString($newaction, "text"),GetSQLValueString($str, "text"), GetSQLValueString($today, "text"), GetSQLValueString($_POST['userID'][0], "text"), GetSQLValueString(trim($_POST['appNum']), "text"));
}
...
$query_Recordset1 = "SELECT `user`.username,`user`.FirstName, `user`.LastName,`user`.Department, `user`.User_Type FROM `user` WHERE `user`.User_Type = 'reviewer'";
...
<tr><td colspan="3"> <div align="center">
 ...
 echo "Reviewer(s) Assigned to this Application</strong></p> <p> </p></div></td></tr>";
...
<td align="center" colspan="3"><span class="style46">To assign reviewer(s) to an IRB application, please select reviewer(s) from the list: </span>
  <p>
...
<select name="userID[]" id="userID[]" multiple size = "5">
<?php 
do {  
...
$query_Recordset3 = "SELECT application.App_Number, application.ProjectTitle, application.Status FROM application WHERE ((application.rev1ID = '".$theValue."' || application.rev2ID = '".$theValue."' || application.rev3ID = '".$theValue."') && (application.Status != 'Approved'))";
