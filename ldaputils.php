...
$_ldap_login_id_output[0] = "utaid";

$_ldap_ppl_output[0] = "displayname"; // full name
$_ldap_ppl_output[1] = "utaemployeetitle"; //rank
$_ldap_ppl_output[2] = "utadeptname"; // dept name
$_ldap_ppl_output[3] = "utaid"; //login id not login name
$_ldap_ppl_output[4] = "sn"; // last name
$_ldap_ppl_output[5] = "givenname"; // first name
$_ldap_ppl_output[6] = "utamiddlename"; // middlename
...
$_ldap_ppla_output[7] = "utaemployeephone"; // Phone number
$_ldap_ppla_output[8] = "mail"; // email id
$_ldap_ppla_output[9] = "utamailalias"; // email alias
...
$_ldap_pplb_output[9] = "utamailalias"; // email alias
$_ldap_pplb_output[10] = "utaemployeeroom"; // room number
$_ldap_pplb_output[11] = "utaemployeebldg"; // building name
$_ldap_pplb_output[12] = "utaemployeecampusbox"; // PO Box number
$_ldap_pplb_output[13] = "utadeptcode"; // department code - People
$_ldap_pplb_output[14] = "utaHomeStreet1"; //  - People
$_ldap_pplb_output[15] = "utaHomeStreet2"; //  - People
$_ldap_pplb_output[16] = "utaHomeCity"; //  - People
$_ldap_pplb_output[17] = "utaHomeState"; //  - People
$_ldap_pplb_output[18] = "utaHomeZip"; //  - People
$_ldap_pplb_output[19] = "homePhone"; //  - People
$_ldap_pplb_output[20] = "cedarid";
//  - People
...
function real_query_info_ldap($db_conn, $input_fields, $output_fields, $limit, $sort_by, $pageno) {
    ...
    if ($ldap = ldap_connect($_ldap_server)) {
        ...
        if ($sr) {
            ...
            if ($count_entries <= ( ($pageno - 1) * $_browse_search_rows_per_page )) $pageno = $max_page;
            if ($pageno > 0) {
            ...
            for ($i = $start_limit; $i < $end_limit; $i++) {
                for ($j = 0; $j < count($output_fields); $j++) {
                    for ($k = 0; $k < count($info[$i][$output_fields[$j]]) - 1; $k++) {
                        if ($k != 0) $output[$i - $start_limit][$j] .= ", ";
...
function real_get_login_id_by_login_name_ldap($db_conn, $login_name) {
    ...
    $input_fields = array();
    $input_fields[0][0] = $_ldap_input_login_name;
    $input_fields[0][1] = $login_name;
...
function real_get_ppl_by_login_id_ldap($db_conn, $login_id) {
    ...
    $input_fields[0][0] = $_ldap_input_login_id;
    $input_fields[0][1] = $login_id;
...
function real_get_ppl_by_name_ldap($db_conn, $last_name, $first_name, $page_no) {
    ...
    $input_fields[0][0] = $_ldap_input_last_name;
    $input_fields[0][1] = $last_name . "*";
    $input_fields[1][0] = $_ldap_input_first_name;
    $input_fields[1][1] = $first_name . "*";
...
function real_search_ldap($key, $value, $_ldap_search_dn) {
    ...
    $input_fields = array();
    $input_fields[0][0] = $key;
    $input_fields[0][1] = $value;
...
function search_ldap_core($input_fields, $output_fields, $limit, $sort_by, $pageno, $_ldap_search_dn) {
    ...
    else echo "LDAP Connect Error<br>\n";
}
?>